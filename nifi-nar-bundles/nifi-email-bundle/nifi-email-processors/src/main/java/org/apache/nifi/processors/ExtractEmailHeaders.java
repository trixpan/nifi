...
public class ExtractEmailHeaders extends AbstractProcessor {
    ...
    public static final PropertyDescriptor CAPTURED_HEADERS = new PropertyDescriptor.Builder()
            .name("Header List")
            .description("COLON separated list of headers to be extracted from the flowfile  content. " +
                    "NOTE the header key is case insensitive and will be matched as lower-case." +
                    " Values will respect email email contents.")
            .required(false)
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) {
        ...
        // May need to think a better way of doing this still without triggering
        // "already in use for an active callback" error.
        if (attributes.size() > 0) {
            ...
            logger.error("Could not parse the flowfile {} as an email, treating as failure", new Object[]{originalFlowFile});
